...
async function writeHtml(filename, content) {
  await ensureDir(config.outputDir);
  const targetFile = path.join(config.outputDir, filename);
...
function buildAttendanceHtmlReport(attendanceResults) {
  const embeddedData = JSON.stringify(attendanceResults).replace(/</g, "\\u003c");
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .stats-line.present { color: var(--present); }
    .stats-line.absent { color: var(--absent); }
    .stats-line.late { color: var(--late); }
    .stats-line.excused { color: var(--excused); }
    .link-btn {
      border: 0;
    ...
    .status-text.present { color: var(--present); }
    .status-text.absent { color: var(--absent); }
    .status-text.late { color: var(--late); }
    .status-text.excused { color: var(--excused); }
    .empty {
      padding: 20px 16px;
...
<body>
  <div class="wrap">
    <section class="hero">
      ...
      <p class="sub">Ringkasan presensi per mata kuliah untuk kebutuhan monitoring admin.</p>
    </section>

    <section class="toolbar">
      <input id="search" type="search" placeholder="Cari course, teacher, tanggal, nama mahasiswa, atau NIM">
  ...
  <script>
    ...
    function statusClass(status) {
      return ["present", "absent", "late", "excused"].includes(status) ? status : "unknown";
    }

    function getCounts(students) {
      const counts = { present: 0, absent: 0, late: 0, excused: 0, unknown: 0 };
      for (const student of students || []) {
        const key = counts[student.status] !== undefined ? student.status : "unknown";
        counts[key] += 1;
      }
      return counts;
    }
    ...
    function buildSummary(data) {
      ...
      const sessions = data.reduce((sum, course) => sum + (course.attendance?.length || 0), 0);
      const students = data.reduce(
        (sum, course) =>
          sum + (course.attendance || []).reduce((acc, session) => acc + (session.absensi?.length || 0), 0),
        0
      );
      const absent = data.reduce(
        (sum, course) =>
          ...
          (course.attendance || []).reduce(
            (acc, session) => acc + (session.absensi || []).filter((item) => item.status === "absent").length,
            0
          ),
        0
      );
    ...
    function render(data) {
      ...
      if (!data.length) {
        app.innerHTML = '<div class="empty">Tidak ada data yang cocok.</div>';
        return;
      }

      const maxWeeks = data.reduce((max, course) => Math.max(max, (course.attendance || []).length), 0);
      const detailRows = [];

      const headerCols = data.map((course) => {
        const teacherText = (course.teachers || []).join("\\n");
        return \`
          <th class="course-col">
            <div class="course-title">\${escapeHtml(course.title)}</div>
            <div class="course-teachers">Teacher:\\n\${escapeHtml(teacherText || "-")}</div>
          </th>
        \`;
      }).join("");

      const bodyRows = Array.from({ length: maxWeeks }, (_, weekIndex) => {
        const cells = data.map((course, courseIndex) => {
          const session = course.attendance?.[weekIndex];
          if (!session) {
            return '<td class="session-empty">-</td>';
          }

          const students = session.absensi || [];
          const counts = getCounts(students);
          const detailId = \`detail-\${courseIndex}-\${weekIndex}\`;
          const studentRows = students.map((student) => \`
            <tr>
              <td>\${escapeHtml(student.name)}</td>
              <td>\${escapeHtml(student.nim)}</td>
              <td><span class="status-text \${statusClass(student.status)}">\${escapeHtml(student.status || "unknown")}</span></td>
            </tr>
          \`).join("");

          detailRows.push(\`
            <tr id="\${detailId}" class="detail-row">
              <td class="detail-cell" colspan="\${data.length + 1}">
                <div class="detail-title">\${escapeHtml(course.title)} - Week \${weekIndex + 1}</div>
                <table class="detail-table">
                  ...
                  <tbody>\${studentRows || '<tr><td colspan="3">Tidak ada data mahasiswa.</td></tr>'}</tbody>
                </table>
              </td>
            </tr>
          \`);
          ...
          return \`
            <td class="session-cell">
              <div class="session-date">\${escapeHtml(session.date || "Week " + (weekIndex + 1))}</div>
              <div class="session-time">\${escapeHtml(session.time || "-")}</div>
              <div class="stats">
                <div class="stats-line present">Present: \${counts.present}</div>
                <div class="stats-line absent">Absent: \${counts.absent}</div>
                <div class="stats-line late">Late: \${counts.late}</div>
                <div class="stats-line excused">Excused: \${counts.excused}</div>
              </div>
              <button class="link-btn" type="button" data-target="\${detailId}">Lihat detail</button>
            </td>
          \`;
        }).join("");
        ...
        return \`
          <tr>
            <td class="week-col">Week \${weekIndex + 1}</td>
            \${cells}
          </tr>
          \${detailRows.splice(0).join("")}
        \`;
      }).join("");
      ...
      app.querySelectorAll(".link-btn").forEach((button) => {
    ...
    function populateProgramOptions(data) {
      const select = document.getElementById("programFilter");
      const currentValue = select.value;
      const programs = Array.from(new Set(data.map((item) => item.program || "Tanpa Prodi"))).sort();
      select.innerHTML =
        '<option value="">Semua Prodi</option>' +
        programs.map((program) => \`<option value="\${escapeHtml(program)}">\${escapeHtml(program)}</option>\`).join("");
    ...
    function applyFilters() {
      ...
      const filtered = rawData
        .filter((course) => !program || (course.program || "") === program)
        .map((course) => {
          const teachers = (course.teachers || []).join(" ").toLowerCase();
          const sessions = (course.attendance || []).map((session) => {
            const absensi = (session.absensi || []).filter((student) => {
              const studentMatch = !query || [
                course.title,
                teachers,
                session.date,
                session.time,
            ...
            if (sessionMatch && !status && absensi.length === 0) {
              return session;
            }

            return { ...session, absensi };
          }).filter((session) => (session.absensi || []).length || !query);

          return { ...course, attendance: sessions };
        })
        .filter((course) => {
          if (!query && !status) return true;
          const courseMatch = [course.title, (course.teachers || []).join(" ")].join(" ").toLowerCase().includes(query);
          return courseMatch || (course.attendance || []).length;
        });
    ...
    populateProgramOptions(rawData);
    render(rawData);
  </script>
</body>
</html>`;
...
async function run() {
  ...
  try {
    if (target === "attendance" || target === "all") {
      ...
      const htmlFile = await writeHtml(
        `attendance-${stamp}.html`,
        buildAttendanceHtmlReport(simplifiedAttendanceResults),
      );
      ...
      console.log(`Attendance viewer saved to ${htmlFile}`);
    }
