...
const DataCapaianPembelajaranTable = ({
  setOpenModalDelete,
  userRole,
  setSelectedItem,
  loading,
      ...
      Cell: ({
        ...
        return (
          <div className="flex flex-row space-x-2">
            <EditIcon
              onClick={() => {
                navigate(`/kurikulum-obe/capaian-pembelajaran/${value.id}`, {
                  state: value,
                });
              }}
            />
          ...
          <div className="p-4 bg-yellow-100 text-yellow-800 border border-yellow-400 rounded-md">
            ...
            {prodiWatch &&
              dataProgramStudi.find((prodi) => prodi.value === prodiWatch)
                .label}
            , belum ada data CPL. Silahkan lengkapi dan buatkan CPL untuk prodi
            terkait.
          </div>
        ) : (
...
export default DataCapaianPembelajaranTable;
