...
import { usePostPenugasanPengajaranByExcel } from '../../hooks/usePenugasanPengajaranByExcel';
...
const PenugasanPengajaranImportExcel = () => {
  ...
  const {
    mutateAsync: postPenugasanPengajaranByExcel,
    isLoading: postPenugasanPengajaranByExcelLoading,
  } = usePostPenugasanPengajaranByExcel();
      ...
      <div>
        {errorMessage ? (
          ...
          <PrimaryButton
            className={`!mt-8 !mb-5`}
            onClick={onSubmit}
            disabled={postPenugasanPengajaranByExcelLoading}
          >
            Simpan Data
          </PrimaryButton>
        )}
        ...
        <table className="w-full mt-6">
          <thead className="bg-primary-400/[0.03] whitespace-nowrap rounded-xl">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold">
                <p className="flex flex-row items-center">Nama Dosen</p>
              </th>
              <th className="px-4 py-3 font-semibold">
                <p className="flex flex-row items-center">NIK</p>
              </th>
              <th className="px-4 py-3 font-semibold">
                <p className="flex flex-row items-center">Inisial</p>
              </th>
              <th className="px-4 py-3 font-semibold">
              ...
              <th className="px-4 py-3 font-semibold">
                <p className="flex flex-row items-center">Surat Penugasan</p>
              </th>
            </tr>
          </thead>
          <tbody>
                  ...
                  .filter((getdata) => {
                    return (
                      (namadosen === '' ||
                        (getdata.nama_dosen
                          .toLowerCase()
                          .includes(namadosen.toLowerCase()))
                        ) &&
                      (matakuliah === '' ||
                        (getdata.mata_kuliah
                          .toLowerCase()
                          .includes(matakuliah.toLowerCase()))
                        )
                    );
                  })
                  .map((filteredData, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">{index + 1}</td>
                      <td className="px-4 py-3">
                        {filteredData.nama_dosen}
                        {/* {filteredData.mahasiswa_detail['nama']} */}
                      </td>
                      <td className="px-4 py-3">
                        {filteredData.nik_dosen}
                        {/* {filteredData.mahasiswa_detail.nim} */}
                      </td>
                      <td className="px-4 py-3">
                        {filteredData.inisial_dosen}
                        {/* {filteredData.mahasiswa_detail.nim} */}
                      </td>
                      <td className="px-4 py-3">{filteredData.mata_kuliah}
                        {/* {filteredData.mahasiswa_detail.prodi_detail.name} */}
                      </td>
                      <td className="px-4 py-3">{filteredData.kode_mk}
                        {/* {filteredData.mahasiswa_detail.angkatan} */}
                      </td>
                      <td className="px-4 py-3">{filteredData.sks}
                        {/* {filteredData.mahasiswa_detail.angkatan} */}
                      </td>
                      <td className="px-4 py-3">{filteredData.surat_penugasan}
              ...
              : excelData
                  ...
                  .map((filteredData, index) => (
                    <tr key={index} className="bg-white border-b text-gray-600">
                      ...
                      <td className="px-4 py-3">
                        {filteredData.Initial}
                      </td>
                      <td className="px-4 py-3">{filteredData.Subject}</td>
                      <td className="px-4 py-3">{filteredData['Subject Short']}</td>
                      <td className="px-4 py-3">{filteredData['Graded Credits']}</td>
                      <td className="px-4 py-3">{filteredData['Surat Penugasan']}</td>
                    </tr>
                  ))}
          </tbody>
        </table>
...
export default PenugasanPengajaranImportExcel;
