...
const ListSuratPenugasan = () => {
  ...
  const getData = async () => {
    try {
      const resp = await dataApi.getSuratPenugasan();
      return resp;
    } catch (error) {
      return [];
    }
        ...
        <>
          <List>
            <ListItem disablePadding>
              <ListItemButton onClick={() => openInNewTab(`${filteredPenugasan[i].files}`)}>
                <ListItemIcon>
                  <InsertDriveFileIcon className="icon" />
                </ListItemIcon>
                <ListItemText>
                  <p>{filteredPenugasan[i].judul}</p>
                </ListItemText>
              </ListItemButton>
              <Box className="row">
                <Button
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value === '') {
                    ...
                    const filtered = penugasan.filter(
                      (el) => el.judul.toLowerCase().includes(e.target.value.toLowerCase()),
                    );
...
export default ListSuratPenugasan;
