...
const ListProgramStudi = () => {
  ...
  const getData = async () => {
    try {
      const resp = await dataApi.getProgramStudi();
      return resp;
    } catch (error) {
      return [];
    }
          ...
          <List>
            <ListItem disablePadding>
              <ListItemButton onClick={() => {
                history.push(`/dashboard/programstudi/${filteredPenugasan[i].id}`);
              }}
              >
                <ListItemIcon>
                  <Book className="icon" />
                ...
                <ListItemText>
                  <p>[{filteredPenugasan[i].kode}] {filteredPenugasan[i].name}</p>
                </ListItemText>
              </ListItemButton>
              <Box className="row">
                <Button
      ...
      <Card>
        ...
        <CardBody>
          <div className="card__title">
            <h5 className="bold-text">Program Studi</h5>
          </div>
...
export default ListProgramStudi;
